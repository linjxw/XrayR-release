...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/wyusgw/XrayR/refs/heads/master/release/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/wyusgw/XrayR/refs/heads/master/release/install.sh) $version
    if [[ $? == 0 ]]; then
